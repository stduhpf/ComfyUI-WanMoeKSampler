...
Instead of guessing the denoising step at which to swap from the high noise model to the low noise model, this node automatically chanage to the low noise model when we reach the diffusion timestep at which the signal to noise ratio is supposed to be 1:1.
There is additionally an option to adjust the CFG throught the denoising process, in both the high and low denoise stages.
